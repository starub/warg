...
class D10DiceSpec extends FlatSpec {

  behavior of "standard 10-sided dice"

  it should "roll non-zero values from 1 to 10 inclusive only" in {

    for (i <- 1 to 10000) {
      var roll = D10Dice.roll()
      assert(roll != 0 && roll >= 1 && roll <= 10)
    }
  }
